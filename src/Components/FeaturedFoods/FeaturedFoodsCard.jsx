...
const FeaturedFoodsCard = ({food}) => {
    const {food_Img, food_name, donator_img, donator_name,food_quantity, location, expired_date, description} = food;
    console.log(food);
    return (
        <div className="">
            <div className="flex gap-5 mb-4 mr-4 bg-gray-50 border border-blue-300 transition duration-300 cursor-pointer rounded-md w-auto p-5">
                ...
                <Link to={""} className="flex items-center justify-center rounded-md focus:outline-none py-2 px-3 text-center bg-blue-800 text-white font-semibold hover:bg-blue-600 transition duration-300">
              View Details
            </Link>
                </div>
            </div>
